<?php
	...
	class usda {
		...
		public function createEml() {
			...
			$tpl = str_replace("{onlineUrl}", 'http://plants.usda.gov/' , $tpl);
			$tpl = str_replace("{logoUrl}", '' , $tpl);
			$tpl = str_replace("{pubDate}", date("Y/m/d") , $tpl);
			$tpl = str_replace("{abstract}", '' , $tpl);
			$tpl = str_replace("{recordLinkUrl}", 'http://plants.usda.gov/java/profile?symbol={ID}' , $tpl);
